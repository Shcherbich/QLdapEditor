...
LDAPMessageQueue* LDAPDeleteRequest::sendRequest()
{
    ...
    int err = ldap_delete_ext(m_connection->getSessionHandle(), m_dn.c_str(), tmpSrvCtrls, tmpClCtrls, &msgID);
    // LDAP_SERVER_DOWN
    ...
    if (err == LDAP_SERVER_DOWN)
    {
        ...
        {
            err = ldap_delete_ext(m_connection->getSessionHandle(), m_dn.c_str(), tmpSrvCtrls, tmpClCtrls, &msgID);
        }
    }
