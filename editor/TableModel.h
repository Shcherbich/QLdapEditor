...
class TableModel : public QAbstractTableModel, public ModelDataAdapter<Value>
{
public:
    ...
    QVariant headerData(int section, Qt::Orientation orientation,
                        int role) const
    {
        if (orientation == Qt::Horizontal && headers.contains(section) &&
    ...
    Qt::ItemFlags flags(const QModelIndex& index) const
    {
        ...
        return ModelDataAdapter<Value>::getFlags(index.column());
    }
    ...
    {
        bool result = ModelDataAdapter<Value>::setData(
                          &*(container.begin() + index.row()), index.column(), value, role);
    ...
    void remove(int row, int count = 1)
    {
        ...
        container.erase(container.begin() + row,
                        container.begin() + row + count - 1);
        endRemoveRows();
    }
    ...
    const value_type& getValue(int pos) const { return container.at(pos); }
